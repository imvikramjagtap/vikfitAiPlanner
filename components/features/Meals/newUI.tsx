...
export default function MealPlanCards() {
  ...
  const sortedMealPlans = [...dailyMealPlans].sort((a, b) => 
  ...
  const MealPlanCard = ({ plan, isLatest = false } : {plan: DailyMealPlan, isLatest?: boolean}) => (
    <Card className={`overflow-hidden ${isLatest ? 'mb-8' : ''}`}>
      ...
      <CardContent>
        <div className="mb-4 flex gap-2">
          ...
          <Badge variant="secondary">
            Total Protein: {plan.totalProtein}
          </Badge>
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      {pastPlans.length > 0 && (
        <div>
          <h2 className="text-2xl font-bold mb-4">Past Diets</h2>
